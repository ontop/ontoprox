...
public class SQLSourceQueryGenerator {
	...
	private static final org.slf4j.Logger log = LoggerFactory
			.getLogger(SQLSourceQueryGenerator.class);
	 ...
	 */
	public SQLSourceQueryGenerator(DBMetadata metadata, SQLDialectAdapter sqladapter) {
	...
	public SQLSourceQueryGenerator(DBMetadata metadata, SQLDialectAdapter sqladapter, boolean sqlGenerateReplace) {
...
//		.put(OBDAVocabulary.SPARQL_COUNT, OBDAVocabulary.XSD_INTEGER, OBDAVocabulary.XSD_INTEGER)
	 ...
	 * {@code #generateQuery(DatalogProgram, List, String, Map, List, Set)}
	 *  @param queryProgram
	 ...
	 * {@code #createViewFrom(Predicate, DBMetadata, Map, DatalogProgram, List)}
	 ...
	 *            <code> query </query>. {@code #DatalogDependencyGraphGenerator}
...
//	}
	...
	private void createViewFrom(Predicate pred, DBMetadata metadata,
		...
		for (CQIE rule : ruleList) {
			...
			List<String> varContainer = getVariableNamesInAtom(cqHead);
...
//			}
	...
	private Set<Variable> getVariableReferencesWithLeftJoin(Function atom) {
		if (atom.isDataFunction()) {
			return new HashSet<>(getVariables(atom));
		}
		if (atom.isBooleanFunction()) {
	...
	private String getSelectClause(List<String> signature, CQIE query,
		...
		while (hit.hasNext()) {
            ...
            // CHANGED FOR SQLSourceQueryGenerator !!
...
//				final String viewName = buildDefaultName(VIEW_PREFIX, tableName, VIEW_ANS_SUFFIX);
//				/**
...
//				final String simpleTableViewName = buildDefaultName(VIEW_PREFIX, tableName, suffix);
...
//						columnName = def.getAttribute(attPos).getName();
//					} else {
//						columnName = def.getAttribute(index+1).getName();
//					}
//
...
//			DataDefinition def = dataDefinitions.get(atom);
//			String columnname = def.getAttribute(column).getName();
...
//	}
    ...
    public static List<Variable> getVariables(Function atom) {
